...
protocol SatSelectionDelegate: AnyObject {
    func onSatChanged()
}

class SPalette: UIView {
    ...
    var satSelectionDelegate: SatSelectionDelegate? = nil
    ...
    private func drawSatPalette(on context: CGContext, pcv: PickedColorValues) {
        ...
        for x in 0..<w {
            let s = CGFloat(x) / wf * pcv.maxValue
            
            // Convert hue to UIColor using HSB color space
            let color = UIColor(hue: pcv.h, saturation: s, brightness: pcv.b, alpha: 1.0)
    ...
    @objc func didDraw(sender: UIDrawGestureRecognizer) {
        ...
        if sender.state == .began || sender.state == .changed {
            if let pcv = pcv {
                pcv.s = max(min(location.x / CGFloat(w) * pcv.maxValue, pcv.maxValue), pcv.minValue)
                
                print("sat = \(pcv.s)")
                
                satSelectionDelegate?.onSatChanged()
    ...
    func moveIndicator() {
        if let pcv = pcv {
            indicator.frame = CGRect(x: CGFloat(w) * (pcv.s / pcv.maxValue) - indicatorSize / 2,
